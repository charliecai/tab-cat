...
**Implementation Snapshot:** The current branch now ships the reviewed phase-1 architecture: `Now / Reading inbox`, `Pinned v1`, IndexedDB-backed article records, article-scoped background jobs, lightweight topic summaries, lifecycle actions (`read / archive / delete / retry`), and an in-page AI settings/debug drawer. The remaining checklist below should be treated as a verification and follow-through list rather than a greenfield build plan.
